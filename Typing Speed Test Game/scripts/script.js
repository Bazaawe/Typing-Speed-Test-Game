...
let timer, maxTime = 60 * 60, timeLeft = maxTime, 
charIndex = mistakes = isTyping = 0;

 function randomParagraph(){
    //getting random number and it'll always less tha the paragraphs length
    let randIndex = Math.floor(Math.random() * paragraphs.length);
    typingText.innerHTML = "";
   // getting random item from the paragraphs array,splitting all characters paragraphs[randIndex].split("").forEach(span => {
...
function initTyping(){
    ...
    if(charIndex < characters.length - 1 && timeLeft > 0){
        ...
        if(typedChar == null){
            ...
            // decrement mistakes only if the charIndex span contains in correct class
